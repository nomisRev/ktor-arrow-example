...
/** UserPersistence implementation based on SqlDelight and BCrypt */
fun userPersistence(usersQueries: UsersQueries, rounds: Int = 10) = object : UserPersistence {
  ...
  override suspend fun insert(
    ...
    return Either.catch { usersQueries.create(hash, username, email) }.fold({ error ->
      if (error is PSQLException && error.sqlState == PSQLState.UNIQUE_VIOLATION.state) {
        shift(UsernameAlreadyExists(username))
      } else {
        shift(Unexpected("Failed to persist user: $username:$email", error))
      }
    }, ::identity)
  }
