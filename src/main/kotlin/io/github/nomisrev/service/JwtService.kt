...
context(EffectScope<UserError>, EffectScope<JwtError>, UserPersistence)
...
// Ambigous context receiver: https://github.com/Kotlin/KEEP/issues/259#issuecomment-1128755717
//  val userId =
...
//    ensureNotNull(jwt.expiresAt().orNull()) { JwtInvalid("exp missing from JWT Token") }
  val userId = jwt.claimValueAsLong("id").orNull() ?: shift(JwtInvalid("id missing from JWT Token"))
  val expiresAt = jwt.expiresAt().orNull() ?: shift(JwtInvalid("exp missing from JWT Token"))
